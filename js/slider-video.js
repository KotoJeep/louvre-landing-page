...
function onPlayerStateChange(event) {
	...
	players.forEach((item) => {
		if (item !== event.target && item.getPlayerState() == YT.PlayerState.PLAYING) item.pauseVideo();
	});
}

$('.video-slider__container').on('beforeChange', function (event, slick, currentSlide, nextSlide) {
...
$('.video-slider__container').on('afterChange', function (event, slick, currentSlide) {
	...
	iframes.forEach((activeIframe) => {
		if (activeIframe.hasAttribute('id') && activeIframe.id.indexOf('player') != -1) return;
